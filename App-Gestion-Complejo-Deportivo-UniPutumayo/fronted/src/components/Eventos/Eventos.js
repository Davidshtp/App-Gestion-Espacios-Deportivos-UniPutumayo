...
import {
  obtenerTodosLosEventosParaAdmin,
  crearEvento,
  actualizarEvento,
  eliminarEvento,
} from "../../Services/eventos/eventosService";
...
export default function GestionEventos() {
  ...
  const handleGuardarEvento = async (formData) => {
    mostrarCargando(
      eventoEnEdicion ? "Actualizando evento..." : "Creando evento...",
    );
    try {
      if (eventoEnEdicion) {
  ...
  return (
    <div className="gestion-eventos-container">
      ...
      {eventos.length === 0 ? (
        <p className="no-eventos-msg">
          No hay eventos para mostrar. ¡Crea uno!
        </p>
      ) : (
              ...
              <div className="evento-info">
                ...
                <p className="evento-fecha">
                  ...
                  {evento.fecha_hora_evento
                    ? new Date(evento.fecha_hora_evento).toLocaleString(
                        "es-ES",
                        {
                          year: "numeric",
                          month: "long",
